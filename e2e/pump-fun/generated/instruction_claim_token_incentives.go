...
// ClaimTokenIncentivesDiscriminator is the discriminator that prefixes the serialized data.
...
func (inst *ClaimTokenIncentives) MarshalWithEncoder(encoder *ag_binary.Encoder) error {
	if err := encoder.WriteBytes(ClaimTokenIncentivesDiscriminator, false); err != nil {
		return err
	}
	return nil
}
...
func (inst *ClaimTokenIncentives) UnmarshalWithDecoder(decoder *ag_binary.Decoder) error {
	if got, err := decoder.ReadNBytes(len(ClaimTokenIncentivesDiscriminator)); err != nil {
		return err
	} else if !bytes.Equal(got, ClaimTokenIncentivesDiscriminator) {
		return fmt.Errorf("invalid discriminator for ClaimTokenIncentives: got %x, want %x", got, ClaimTokenIncentivesDiscriminator)
	}
	return nil
}
