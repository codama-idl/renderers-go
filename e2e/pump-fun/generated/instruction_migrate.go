...
// MigrateDiscriminator is the discriminator that prefixes the serialized data.
...
func (inst *Migrate) MarshalWithEncoder(encoder *ag_binary.Encoder) error {
	if err := encoder.WriteBytes(MigrateDiscriminator, false); err != nil {
		return err
	}
	return nil
}
...
func (inst *Migrate) UnmarshalWithDecoder(decoder *ag_binary.Decoder) error {
	if got, err := decoder.ReadNBytes(len(MigrateDiscriminator)); err != nil {
		return err
	} else if !bytes.Equal(got, MigrateDiscriminator) {
		return fmt.Errorf("invalid discriminator for Migrate: got %x, want %x", got, MigrateDiscriminator)
	}
	return nil
}
