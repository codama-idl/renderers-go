...
// SellDiscriminator is the discriminator that prefixes the serialized data.
...
func (inst *Sell) MarshalWithEncoder(encoder *ag_binary.Encoder) error {
	if err := encoder.WriteBytes(SellDiscriminator, false); err != nil {
		return err
	}
...
func (inst *Sell) UnmarshalWithDecoder(decoder *ag_binary.Decoder) error {
	if got, err := decoder.ReadNBytes(len(SellDiscriminator)); err != nil {
		return err
	} else if !bytes.Equal(got, SellDiscriminator) {
		return fmt.Errorf("invalid discriminator for Sell: got %x, want %x", got, SellDiscriminator)
	}
	if err := decoder.Decode(&inst.Amount); err != nil {
		return err
	}
	if err := decoder.Decode(&inst.MinSolOutput); err != nil {
		return err
	}
	return nil
}
